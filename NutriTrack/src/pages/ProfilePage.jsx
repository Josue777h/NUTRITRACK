...
function ProfilePage({ onLogout, defaultTab }) {
    ...
    const [personalForm, setPersonalForm] = useState({
        ...
        documentId: isNutri ? (profile.documentId || "") : (currentPatient?.documentId || currentPatient?.document_id || profile.documentId || ""),
    ...
    useEffect(() => {
        if (currentPatient) {
            setPersonalForm(p => ({
                ...
                documentId: currentPatient.documentId || currentPatient.document_id || "",
    ...
    const handleSavePersonal = async (e) => {
        ...
        await updateProfile({
            ...
            documentId: personalForm.documentId
        });

        if (!isNutri && currentPatient) {
            updatePatient(currentPatient.id, {
                ...
                phone: personalForm.phone,
                documentId: personalForm.documentId,
                document_id: personalForm.documentId,
    ...
    const navItems = isNutri ? [
        { key: "personal",  label: "Datos de la Cuenta",  icon: "bi-person-fill",       desc: "Nombre, cédula, contacto" },
        { key: "seguridad", label: "Seguridad y Cuentas", icon: "bi-shield-lock-fill", desc: "Contraseña y acceso" }
    ] : [
        { key: "personal",  label: "Información Personal", icon: "bi-person-fill",       desc: "Nombre, cédula, contacto" },
                ...
                {isNutri && (
                    <div className="pf-quick-stats">
                        <div className="pf-qstat">
                            ...
                            <span>{personalForm.documentId || "—"}</span>
                            <small>Identificación / DNI</small>
                        </div>
                        <div className="pf-qstat">
                            <i className="bi bi-telephone-fill" />
                            <span style={{ fontSize: "0.75rem" }}>{personalForm.phone || "—"}</span>
            ...
            <main className="pf-content">
                ...
                {activeBlock === "personal" && (
                    <form onSubmit={handleSavePersonal} className="pf-panel" noValidate>
                        <SectionTitle
                            icon="bi-person-fill"
                            title={isNutri ? "Datos de la Cuenta" : "Información Personal"}
                            subtitle="Actualiza tus datos de contacto e identificación básica en la plataforma."
                        />
                        ...
                        <div className="pf-form-grid">
                            ...
                            <FieldGroup label="Cédula / DNI / Documento" icon="bi-card-text">
                                <input
                                    type="text"
                                    value={personalForm.documentId}
                                    onChange={(e) => setPersonalForm((p) => ({ ...p, documentId: e.target.value }))}
                            ...
                            <FieldGroup label="Teléfono de contacto / WhatsApp" icon="bi-telephone">
                                <input
                                    type="tel"
